    ...
    public function wpc_set_reservation_title( $data, $postarr ) {
        ...
        if ( is_admin() && 'wpc_reservation' == $data['post_type'] && isset($postarr['wpc_email']) && $postarr['wpc_email'] !=='' ) {
            ...
            $wpc_value_arr = [
                ...
                get_post_meta( $postarr['ID'], 'wpc_booking_date', true ) . esc_html__('Start time :', 'wpcafe') .
                get_post_meta( $postarr['ID'], 'wpc_from_time', true ) . esc_html__('End time :', 'wpcafe') .
                get_post_meta( $postarr['ID'], 'wpc_to_time', true ),
                date( 'Y-m-d H:i:s' ),
                $invoice_no,
                get_post_meta( $postarr['ID'], 'wpc_branch', true ),
            ];
